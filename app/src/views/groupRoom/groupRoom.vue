 ...
 * @description: 聊天室核心页面
 * @LastEditors: hua
 * @LastEditTime: 2020-03-19 15:28:41
 -->
<template>
          ...
          <div class="format_time" v-if="index>0 && key.created_at> parseInt(msgList[index-1].created_at)+60">{{formatTime(key.created_at)}}</div>
            <div class="chat-item" v-if="key.user_id == userInfo.id">
              <div class="mychat">
                ...
                <div v-else-if="key.type == CHAT_NOTIFY" class="rawMsg">{{JSON.parse(key.msg)['msg']}}</div>
    ...
    <div class="address-book-list" v-if="addressBookShow" >  
        <dl v-for = "num in 26" :key="num">
            <dt :ref="String.fromCharCode(64+num)">{{String.fromCharCode(64+num)}}</dt>
              <dd v-for="(item, index) in adderssBookList" :key="index" v-if="String.fromCharCode(64+num) == item.users.first_word" @click="handleChooseUser(item.users)"> <!-- 循环 -->
                  <a>
        ...
        <dl>
            <dt ref="#">#</dt>
              <dd  v-for="(item, index) in adderssBookList" :key="index" v-if="item.users.first_word == '#'" @click="handleChooseUser(item.users)"> <!-- 循环 -->
                  <a>
                    <vImg :imgUrl="item.users.head_img"/>
                    {{item.users.nick_name}}
                  </a>
...
export default {
  ...
  computed: {
    ...mapGetters(["msgList", "currentRoomUuid", "currentRoomName", "currentRoomType","userInfo", "htmlFontSize", "currentRoomSaveAction","RECORD","TEXT","RESEND","IMG","FILE","LOADING","SUCCESS","FAIL","CHAT_NOTIFY"])
  },
  data() {
    return {
      uuidVal: "",
  ...
  methods: {
    ...
    handleChooseUser(user){
      ...
      chatSend({
        data: {
          msg: JSON.stringify({msg:msg,user:user, selfUser:this.userInfo}),
          room_uuid: this.currentRoomUuid,
